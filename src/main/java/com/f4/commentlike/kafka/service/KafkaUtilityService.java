package com.f4.commentlike.kafka.service;

import com.f4.commentlike.avro.EventEnvelope;
import com.f4.commentlike.service.dto.LikeDTO;
import com.f4.commentlike.kafka.handler.EventDispatcher;
import com.f4.commentlike.kafka.runner.KafkaJobRunner;
import com.f4.commentlike.kafka.util.AvroConverter;
...
public class KafkaUtilityService {
    ...
    public String producer_prepareAndAttemptDirectSendReelEvent(
        ...
        try {
            ...
            EventEnvelope envelope = AvroConverter.createLikeEvent(eventName, null);
            ...
            retryTemplate.execute(context -> {
                ...
                try {
                    ...
                    if (avroMessage.getPayload() != null) {
                        ...
                        serviceLikeDTO = consumer_mapAvroToServiceLikeDTO(avroMessage.getPayload());

                        if (serviceLikeDTO != null) {
                            // Prepare envelope for dispatcher
                            com.f4.commentlike.kafka.handler.EventEnvelope<JsonNode> envelopeForDispatcher = consumer_prepareEventEnvelopeForDispatcher(
                                    eventName, serviceLikeDTO);
                            ...
                            CONSUMER_LOG.warn("Service LikeDTO was null after mapping for event: {}", eventName);
                        }
                    } else {
                    ...
                    // SSE dispatch
                    consumer_dispatchToSseClients(eventName, serviceLikeDTO, emitters);
    ...
    // existing helper methods: consumer_mapAvroToServiceLikeDTO,
    ...
    public void consumer_dispatchToSseClients(String eventName, LikeDTO reelDTO, Map<String, SseEmitter> emittersMap) {
    ...
    // consumer_mapAvroToServiceLikeDTO now delegates to AvroConverter
    public LikeDTO consumer_mapAvroToServiceLikeDTO(com.f4.commentlike.avro.LikeDTO avroPayload) {
        CONSUMER_LOG.debug(
                "KafkaUtilityService: Delegating Avro to Service DTO conversion to AvroConverter for payload: {}",
                avroPayload);
        try {
            return AvroConverter.convertToServiceLikeDTO(avroPayload);
        } catch (Exception e) {
            CONSUMER_LOG.error(
    ...
    public com.f4.commentlike.kafka.handler.EventEnvelope<JsonNode> consumer_prepareEventEnvelopeForDispatcher(
            String eventName, LikeDTO serviceDto) {
        if (serviceDto == null) {
            ...
            return new com.f4.commentlike.kafka.handler.EventEnvelope<>(eventName, null);
        }
        try {
            ...
            return new com.f4.commentlike.kafka.handler.EventEnvelope<>(eventName, reelJsonNode);
